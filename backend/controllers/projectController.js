...
exports.getAllProjectsUser = async (req, res) => {
  try {
    const projects = await Project.find({
      ...
      .populate(
        "tasks",
        "title description status dateDebut dateTerminee createdBy doneBy archived"
      )
      .exec();
...
exports.addMember = async (req, res) => {
  try {
    ...
    if (project.members.includes(user._id)) {
      return res
        .status(400)
        .json({ message: "User already added to the project" });
    }

    if (email === req.user.email) {
      return res
        .status(400)
        .json({ message: "You cannot add yourself to the project" });
    }
...
exports.removeMember = async (req, res) => {
  try {
    ...
    member = await User.findOne({ email }).select("_id");
...
exports.getTotalCollaboratorsCount = async (req, res) => {
  try {
    ...
    projects.forEach((project) => {
      project.members.forEach((collaboratorId) =>
        collaboratorsSet.add(collaboratorId.toString())
      );
    });
